...
const readings = [
  { title: "Amor", text: "Conectá con tu corazón y descubrí lo que viene.", image: "/assets/reading-love-v1.webp", alt: "Corazón luminoso entre nubes doradas" },
  { title: "Trabajo", text: "Claridad para tus decisiones profesionales.", image: "/assets/reading-work-v1.webp", alt: "Cartas de Tarot sobre terciopelo negro" },
  { title: "Propósito", text: "Encontrá tu camino y alineá tu energía.", image: "/assets/reading-purpose-v1.webp", alt: "Luna dorada reflejada sobre agua oscura" },
  { title: "Lectura general", text: "Una mirada completa a tu presente y futuro.", image: "/assets/reading-general-v1.webp", alt: "Brújula astrológica antigua con cristales" },
];
...
export default function Home() {
  ...
  return <>
    ...
    <main>
      ...
      <section className="readings section" id="lecturas"><div className="section-heading"><p className="eyebrow">Encontrá tu respuesta</p><h2>Lecturas de Tarot</h2><p>Un espacio para cada etapa de tu vida.</p></div><div className="cards">{readings.map((reading) => <article className="reading-card reveal" key={reading.title}><img className="reading-art" src={reading.image} alt={reading.alt} loading="lazy" /><div className="reading-content"><h3>{reading.title}</h3><p>{reading.text}</p><button className="card-link" onClick={() => openReading(reading.title)}>Saber más</button></div></article>)}</div></section>
      ...
      <section className="booking" id="reservar"><div><p className="eyebrow">Tu momento</p><h2>¿Lista para tu lectura?</h2><p>Dale espacio a las respuestas que tu alma ya conoce.</p></div><a className="button button-gold" href="https://www.tiktok.com/@emiliamarsicano" target="_blank" rel="noopener noreferrer">▣ Reservá tu lectura <span>→</span></a></section>
    </main>
